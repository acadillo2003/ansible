...
class Netconf(object):
    ...
    def connect(self, **kwargs):
        host = self.module.params['host']
        port = self.module.params.get('port') or 2022

        username = self.module.params['username']
        password = self.module.params['password']

        self.manager = manager.connect(host=host, port=port, username=username,
    ...
    def configure(self, data):
        self.manager.edit_config(data, format='xml', target='running')
...
class NetworkModule(AnsibleModule):
    ...
    def connect(self):
        if self.params['transport'] == 'netconf':
            self.connection = Netconf(self)
    ...
    def configure(self, commands):
        if self.params['transport'] == 'netconf':
            self.connection.configure(commands)
        else:
    ...
    def get_config(self):
        if self.params['transport'] == 'cli':
            ...
            return self.connection.manager.get_config(source='running')
...
def get_module(**kwargs):
    ...
    if not HAS_PARAMIKO and module.params['transport'] == 'cli':
        module.fail_json(msg='paramiko is required but does not appear to be installed')

    if not HAS_NCC and module.params['transport'] == 'netconf':
